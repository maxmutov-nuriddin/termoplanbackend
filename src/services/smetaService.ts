import { ISmeta, ISmetaItem } from '../models/Smeta';
import { DEFAULT_PRICES_UZS } from '../config/constants';
import { HeatingCalculationResult } from '../algorithms/heatingEngine';
import { CoolingCalculationResult } from '../algorithms/coolingEngine';

export function generateSmetaForProject(
  projectId: any,
  ...
  const acUnits: Record<number, number> = {}; // btu -> count
  ...
  items.push({
    ...
    unitPriceUzs: DEFAULT_PRICES_UZS.PEX_PIPE_PER_M,
    totalPriceUzs: Math.round(totalPipeLength * DEFAULT_PRICES_UZS.PEX_PIPE_PER_M),
    isOptional: false,
  });
  ...
  items.push({
    ...
    unitPriceUzs: DEFAULT_PRICES_UZS.COLLECTOR_PER_PORT,
    totalPriceUzs: Math.max(2, totalManifoldPorts) * DEFAULT_PRICES_UZS.COLLECTOR_PER_PORT,
    isOptional: false,
  });
  ...
  items.push({
    ...
    unitPriceUzs: DEFAULT_PRICES_UZS.EPS_INSULATION_PER_M2,
    totalPriceUzs: Math.round(totalInsulationArea * DEFAULT_PRICES_UZS.EPS_INSULATION_PER_M2),
    isOptional: false,
  });
  ...
  items.push({
    ...
    unitPriceUzs: DEFAULT_PRICES_UZS.DAMPING_TAPE_PER_M,
    totalPriceUzs: Math.round(totalDampingTapeLength * DEFAULT_PRICES_UZS.DAMPING_TAPE_PER_M),
    isOptional: false,
  });
  ...
  items.push({
    ...
    unit: 'dona',
    quantity: clipsCount,
    unitPriceUzs: DEFAULT_PRICES_UZS.PIPE_CLIPS_PER_UNIT,
    totalPriceUzs: clipsCount * DEFAULT_PRICES_UZS.PIPE_CLIPS_PER_UNIT,
    isOptional: false,
  });
  ...
  if (totalServoCount > 0) {
    items.push({
      ...
      unitPriceUzs: DEFAULT_PRICES_UZS.SERVO_ACTUATOR_PER_UNIT,
      totalPriceUzs: totalServoCount * DEFAULT_PRICES_UZS.SERVO_ACTUATOR_PER_UNIT,
      isOptional: false,
    });
    ...
    items.push({
      ...
      unitPriceUzs: DEFAULT_PRICES_UZS.ROOM_THERMOSTAT_PER_UNIT,
      totalPriceUzs: Math.min(totalServoCount, roomsCalculations.length) * DEFAULT_PRICES_UZS.ROOM_THERMOSTAT_PER_UNIT,
      isOptional: false,
    });
  }
  ...
  items.push({
    ...
    unit: 'komplekt',
    quantity: 1,
    unitPriceUzs: finalBoilerKw * DEFAULT_PRICES_UZS.BOILER_PER_KW,
    totalPriceUzs: finalBoilerKw * DEFAULT_PRICES_UZS.BOILER_PER_KW,
    isOptional: true,
  });
  ...
  for (const [btuStr, count] of Object.entries(acUnits)) {
    ...
    const unitPrice = (DEFAULT_PRICES_UZS as any)[`AC_${btu}_BTU`] || DEFAULT_PRICES_UZS.AC_9000_BTU;
    items.push({
      id: `ac_unit_${btu}`,
      category: 'cooling',
  ...
  items.push({
    ...
    unitPriceUzs: DEFAULT_PRICES_UZS.COPPER_PIPE_KIT_PER_M,
    totalPriceUzs: Math.max(4, Math.round(totalCopperLength)) * DEFAULT_PRICES_UZS.COPPER_PIPE_KIT_PER_M,
    isOptional: false,
  });
  ...
  items.push({
    ...
    unitPriceUzs: DEFAULT_PRICES_UZS.OUTDOOR_BRACKET_SET,
    totalPriceUzs: totalAcCount * DEFAULT_PRICES_UZS.OUTDOOR_BRACKET_SET,
    isOptional: false,
  });
  ...
  items.push({
    ...
    unitPriceUzs: DEFAULT_PRICES_UZS.INSTALLATION_WORK_HEATING_PER_M2,
    totalPriceUzs: Math.round(totalHeatingLaborArea * DEFAULT_PRICES_UZS.INSTALLATION_WORK_HEATING_PER_M2),
    isOptional: true,
  });
  ...
  items.push({
    ...
    unit: 'dona',
    quantity: totalAcCount,
    unitPriceUzs: DEFAULT_PRICES_UZS.INSTALLATION_WORK_AC_PER_UNIT,
    totalPriceUzs: totalAcCount * DEFAULT_PRICES_UZS.INSTALLATION_WORK_AC_PER_UNIT,
    isOptional: true,
  });
