import { IPoint, IFurniture } from '../models/Room';
 ...
 */
export function generateUnderfloorPipeRoutes(
  roomWidthM: number,
  roomHeightM: number,
  stepColdM: number = HEATING_CONSTANTS.STEP_COLD_ZONE,
  stepComfortM: number = HEATING_CONSTANTS.STEP_COMFORT_ZONE,
  stepSunnyM: number = HEATING_CONSTANTS.STEP_SUNNY_ZONE,
  manifoldPos: IPoint = { x: 0.2, y: 0.2 },
  furniture: IFurniture[] = [],
  isSunnyRoom: boolean = false
): CircuitPath[] {
  ...
  const maxCircuitLen = HEATING_CONSTANTS.MAX_LOOP_LENGTH_M;
