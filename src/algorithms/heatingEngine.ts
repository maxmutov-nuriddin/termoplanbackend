import { IRoom, IWall, IWindow } from '../models/Room';
import { IProject } from '../models/Project';
import { UZBEKISTAN_CLIMATE_DATA } from '../config/climateData';
import { HEATING_CONSTANTS } from '../config/constants';
import { analyzeInsolation, InsolationAnalysis } from './insolationCalculator';
import { generateUnderfloorPipeRoutes, CircuitPath } from './routePlanner';
...
export function calculateRoomHeating(
  room: IRoom | any,
  project: IProject | any
): HeatingCalculationResult {
  ...
  const specificLossWM2 = Math.round(HEATING_CONSTANTS.BASE_HEAT_LOSS_W_M2 * wallThermalFactor * insulationFactor * insolation.heatingMultiplier);
  ...
  const totalWindowWidth = room.windows?.reduce((acc: number, w: IWindow) => acc + (w.width || 1.4), 0) || 1.4;
  ...
  const circuits = generateUnderfloorPipeRoutes(
    roomWidthM,
    roomHeightM,
    HEATING_CONSTANTS.STEP_COLD_ZONE,
    HEATING_CONSTANTS.STEP_COMFORT_ZONE,
    HEATING_CONSTANTS.STEP_SUNNY_ZONE,
    manifoldPos,
    room.furniture || [],
    isSunny
  );
  ...
  const pipeLengthWithReserve = Math.round(finalPipeLength * (1 + HEATING_CONSTANTS.RESERVE_PERCENTAGE));
  ...
  let screedRecommendation: 'classic_concrete' | 'dry_lightweight' = 'classic_concrete';
